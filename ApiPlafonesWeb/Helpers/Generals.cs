    ...
    public class Generals : ControllerBase
    {
        ...
        public bool Create(Object model, string nameModel)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (nameModel == nameof(UneteModel).ToString())
                {
                    UneteModel uneteModels = new UneteModel();
                    uneteModels = (UneteModel)model;

                    db.UneteModels.Add(uneteModels);
                    db.SaveChanges();
                }

                if (nameModel == nameof(RequieresInstalacionModel).ToString())
                {
                    RequieresInstalacionModel instalacionModel = new RequieresInstalacionModel();
                    instalacionModel = (RequieresInstalacionModel)model;

                    db.RequieresInstalacionModels.Add(instalacionModel);
                    db.SaveChanges();
                }
        ...
        public bool SendEmailSMTP(EmailDto e)
        {
            ...
            try
            {
                ...
                Body += @"<center><div style=""background-color:#01426e;""><br/><img src=""https://media-exp1.licdn.com/dms/image/C4E1BAQFxvPzKnDTMTw/company-background_10000/0?e=2159024400&v=beta&t=MHJ6rinyORIwSmYa4PucInh7e2i3cpLVZvRSaTIJ_1c"" width=""320"" height=""80""></div></center>";
        ...
        public List<ProductosModel> GetUrlMetaProd(List<ProductosModel> products)
        {
            List<ProductosModel> productList = new List<ProductosModel>();
            foreach (var product in products.Where(x => x.EstatusBit == true))
            {
